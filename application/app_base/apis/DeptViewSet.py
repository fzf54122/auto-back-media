...
class DeptViewSet(CustomViewSet,
    ...
    async def create(self, data: Any = Body(...)) -> CoreResponse:
        ...
        if parent_id != 0:
            # 验证父部门是否存在
            parent_dept = await self.queryset.get_or_none(id=parent_id)
            if not parent_dept:
                return CoreResponse(code=400, status="error", msg="父部门不存在")

        new_instance = await self.queryset.create(**self.input_data(data))
        # 创建部门关系
    ...
    async def update(self, uuid: Any, data: Any = Body(...)) -> CoreResponse:
        ...
        # 更新普通字段
        await instance.update_from_dict(self.input_data(data)).save()
        # 更新部门闭包关系
        await service.update_dept_closure(instance)
        # 返回更新后的数据
        result = await instance.to_dict()  # 使用 CoreModel 的 to_dict
